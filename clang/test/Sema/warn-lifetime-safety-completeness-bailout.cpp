...
// RUN: %clang_cc1 -fsyntax-only -Wlifetime-safety-completeness -lifetime-safety-max-cfg-blocks=1 -verify=bailout %s
...
// the bailout completeness warning surfaces the gap so it is not silent under
// the "safe programming model". Without a limit (third RUN) there is no bailout
// and no diagnostics. The warning is also reachable via the -completeness
// umbrella (second RUN).
