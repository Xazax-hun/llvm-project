...
/// Returns the derived class \p S converts to, or null.
///
/// A base-to-derived conversion is never implicit, so this is always a written
/// cast -- but which kind does not matter, and neither does what is converted: a
/// `reinterpret_cast` or a C-style cast reaches the same subobject a `static_cast`
/// does, and the operand may be `this`, a parameter, or a pointer loaded from the
/// heap. So this asks only about the TYPES.
///
/// `dynamic_cast` is exempt, and is the way to write a checked downcast inside a
/// constructor or destructor: there the object is treated as being of that
/// constructor's or destructor's own class, so the conversion is well defined and
/// simply fails.
static const CXXRecordDecl *downcastTarget(const Stmt *S) {
  const auto *CE = dyn_cast<CastExpr>(S);
  if (!CE || CE->getCastKind() == CK_Dynamic)
    return nullptr;
  ...
  if (To->getCanonicalDecl() == From->getCanonicalDecl())
    return nullptr;
  return To->isDerivedFrom(From) ? To : nullptr;
}

/// Finds a base-to-derived conversion, or a call to a function that declares one,
/// anywhere in \p Body.
///
/// Callees are NOT followed. A callee that performs a conversion is required to be
/// marked in its own right, and that requirement is checked wherever it is defined
/// -- which is what makes this hold with no body but this one visible, and hence
/// across translation units.
struct DowncastReach {
  const Stmt *Cast = nullptr;            ///< the conversion, if found directly
  const CXXRecordDecl *Target = nullptr; ///< what it converts to
  const CallExpr *Call = nullptr;        ///< a call to a marked function
  const FunctionDecl *Callee = nullptr;

  explicit operator bool() const { return Cast || Call; }
};

static DowncastReach findDowncastReach(const Stmt *Body) {
  DowncastReach R;
  if (!Body)
    return R;
  llvm::SmallVector<const Stmt *, 32> Work{Body};
  while (!Work.empty()) {
    const Stmt *S = Work.pop_back_val();
    if (!S)
      continue;
    if (const CXXRecordDecl *To = downcastTarget(S)) {
      R.Cast = S;
      R.Target = To;
      return R; // a direct conversion is the more precise report
    }
    if (const auto *CE = dyn_cast<CallExpr>(S))
      if (const FunctionDecl *FD = CE->getDirectCallee())
        if (FD->hasAttr<DowncastsAttr>() && !R.Call) {
          R.Call = CE;
          R.Callee = FD;
        }
...
/// Reports a constructor or destructor that can reach a base-to-derived
/// conversion, and any other function that can reach one without saying so.
///
/// Inside a constructor or destructor of `C`, the derived part of the object has
/// either not been constructed yet or has already been destroyed, so referring to
/// a member or base outside `C`'s own subobject is undefined ([class.cdtor]). By
/// the time a base destructor runs, every heap buffer a derived member owned has
/// been freed -- so a base destructor reading derived state is a use-after-free,
/// and nothing at the read distinguishes it from a live object. The analysis models
/// `this` as a live complete object, so no loan into it ever expires.
///
/// Chasing where the conversion happens does not work: it may be several calls
/// away, applied to a parameter or to a pointer read from the heap, in a body that
/// lives in another translation unit. So the hazard is DECLARED instead. A function
/// that performs a conversion, or calls a function marked '[[clang::downcasts]]',
/// must be marked itself; checked at each definition, that requirement propagates
/// with no body but the one at hand visible.
static void LifetimeSafetyDowncastsAnalysis(Sema &S, TranslationUnitDecl *TU) {
  DiagnosticsEngine &Diags = S.getDiagnostics();
  if (Diags.isIgnored(diag::warn_lifetime_safety_cdtor_downcast, SourceLocation()))
    return;
  llvm::TimeTraceScope TimeProfile("LifetimeSafetyDowncasts");

  struct Walker : DynamicRecursiveASTVisitor {
    Sema &S;
    explicit Walker(Sema &S) : S(S) {
      // A lambda's call operator is implicit, but its body is written inside the
      // enclosing function and reached through that function's statements.
      ShouldVisitImplicitCode = false;
      // CRTP, where this mistake mostly lives, is entirely templates -- and a
      // dependent pattern's cast may not be recognizable until the argument is
      // known.
      ShouldVisitTemplateInstantiations = true;
    }
    ...
    bool VisitFunctionDecl(FunctionDecl *FD) override {
      ...
      if (S.SourceMgr.isInSystemHeader(FD->getLocation()))
        return true;
      // Only INSTANTIATED bodies. In a dependent pattern the conversion cannot be
      // recognized at all -- whether `D` derives from `Lifecycle<D>` is not known
      // until `D` is -- and a pattern that is never instantiated never runs. Every
      // instantiation is visited, so nothing is lost, and a template is not
      // reported once for its pattern and again for each instantiation.
      if (FD->isDependentContext())
        return true;
      DowncastReach R = findDowncastReach(FD->getBody());
      if (!R)
        return true;

      // A constructor or destructor is where the conversion is actually unsafe,
      // and no marker makes it safe -- so report the hazard rather than a missing
      // marker.
      bool IsCtor = isa<CXXConstructorDecl>(FD);
      if (IsCtor || isa<CXXDestructorDecl>(FD)) {
        const CXXRecordDecl *Self = cast<CXXMethodDecl>(FD)->getParent();
        unsigned Kind = IsCtor ? 0 : 1;
        if (R.Cast)
          S.Diag(R.Cast->getBeginLoc(),
                 diag::warn_lifetime_safety_cdtor_downcast)
              << Kind << Self << R.Target;
        else
          S.Diag(R.Call->getExprLoc(),
                 diag::warn_lifetime_safety_cdtor_calls_downcasts)
              << Kind << Self << R.Callee;
        return true;
      }

      // Anywhere else the conversion is fine; not SAYING so is the problem, since
      // a constructor or destructor several frames up has only the declaration to
      // go on.
      if (FD->hasAttr<DowncastsAttr>())
        return true;
      if (R.Cast) {
        S.Diag(FD->getLocation(), diag::warn_lifetime_safety_missing_downcasts)
            << FD;
        S.Diag(R.Cast->getBeginLoc(), diag::note_lifetime_safety_downcast_here)
            << R.Target;
      } else {
        S.Diag(FD->getLocation(),
               diag::warn_lifetime_safety_missing_downcasts_call)
            << FD << R.Callee;
      }
      return true;
    }
...
void clang::sema::AnalysisBasedWarnings::IssueWarnings(
  ...
  if (S.getLangOpts().CPlusPlus) {
    ...
    LifetimeSafetyDowncastsAnalysis(S, TU);
  }
}
