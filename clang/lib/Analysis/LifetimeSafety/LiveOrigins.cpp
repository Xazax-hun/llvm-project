...
public:
  ...
  ///
  /// Loans only ever enter an origin through a fact naming it as a destination --
  /// an Issue, an OriginFlow destination, a Projection, or a dynamic store's
  /// resolved destination -- and every one of those makes the origin persistent
  /// when it appears in another block (see FactManager::computePersistentOrigins).
  /// So a block-local origin holds no loans anywhere outside its own block, and
  /// every consumer of liveness intersects it with the origin's loans. Dropping
  /// these here keeps them out of the joins and out of the block-entry state
  /// comparison.
