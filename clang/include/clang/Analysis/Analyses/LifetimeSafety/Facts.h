...
public:
  enum class Kind : uint8_t {
    ...
    /// loan set.
    OriginFlow,
    /// Loans held by an origin are projected in place: each loan's access path
    /// is extended by a path element. If `obj` holds loan `{x}`, then reading
    /// `obj.field` projects `{x}` with `field` to `{x.field}`.
    Projection,
...
class InvalidateOriginFact : public Fact {
  ...
  bool Assumed;
  /// True when the invalidation ENDS THE OBJECT'S LIFETIME (`delete`, `free`,
